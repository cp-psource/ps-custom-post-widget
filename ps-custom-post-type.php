...
class RcptWidget extends WP_Widget {
	...
	function form( $instance ) {
		...
		$html = '<p>';
		$html .= '<label for="' . $this->get_field_id( 'title' ) . '">' . __( 'Titel:', 'rcpt' ) . '</label>';
		...
		$html .= '<p>' .
		         ...
		         ' <label for="' . $this->get_field_id( 'featured_image' ) . '">' . __( 'Beschränke auf Beiträge mit Beitragsbildern', 'rcpt' ) . '</label> ' .
		         '</p>';
		...
		$html .= '<p>' .
		         ...
		         ' <label for="' . $this->get_field_id( 'show_title' ) . '">' . __( 'Titel zeigen', 'rcpt' ) . '</label> ' .
		         '</p>';
		...
		$html .= '<p>&nbsp;&nbsp;&nbsp;&nbsp;<small>' .
		         ...
		         ' <label for="' . $this->get_field_id( 'titles_as_links' ) . '">' . __( 'Titel und Links zu Beiträgen', 'rcpt' ) . '</label> ' .
		         '</small></p>';

		$html .= '<p>' .
		         '<input type="checkbox" name="' . $this->get_field_name( 'show_body' ) . '" id="' . $this->get_field_id( 'show_body' ) . '" value="1" ' . ( $show_body ? 'checked="checked"' : '' ) . '/>' .
		         ' <label for="' . $this->get_field_id( 'show_body' ) . '">' . __( 'Auszüge zeigen', 'rcpt' ) . '</label> ' .
		         '</p>';
		...
		$html .= '<p>' .
		         ...
		         ' <label for="' . $this->get_field_id( 'show_dates' ) . '">' . __( 'Veröffentlichungsdatum zeigen', 'rcpt' ) . '</label> ' .
		         '</p>';
		...
		$html .= '<p>' .
		         ...
		         ' <label for="' . $this->get_field_id( 'show_thumbs' ) . '">' . __( 'Beitragbilder zeigen <small>(falls verfügbar)</small>', 'rcpt' ) . '</label> ' .
		         '</p>';

		// Custom fields
		$id = sprintf( 'rcpt-custom_fields-%04d-%04d-%04d-%04d', rand(), rand(), rand(), rand() );
		$html .= '<p><a href="#toggle" class="rcpt-toggle_custom_fields" id="' . $id . '-handler">' . __( 'Zeige/Verberge Benutzerdefinierte Felder', 'rcpt' ) . '</a></p>';
		$html .= '<div class="rcpt-show_custom_fields" id="' . $id . '" style="display:none">';
		$html .= '<h5>' . __( 'Benutzerdefinierte Felder', 'rcpt' ) . '</h5>';
		$html .= '<p>';
		...
		if ( $_fields ) {
			foreach ( $_fields as $field ) {
				...
				$shown_fields[] = '<label for="' . $this->get_field_id( 'fields' ) . '-' . $field . '">' . sprintf( __( 'Etikett für &quot;%s&quot;', 'rcpt' ), $field ) . '</label>' .
				                  '<input type="text" class="widefat" name="' . $this->get_field_name( 'fields' ) . '[' . $field . ']" id="' . $this->get_field_id( 'fields' ) . '-' . $field . '" value="' . $value . '" />' .
				                  '';
			}
		}
		if ( $shown_fields ) {
			$html .= '<small><em>' . __( "Felder ohne zugehörige Bezeichnung werden in der Widget-Ausgabe nicht angezeigt", 'rcpt' ) . '</em></small><br />';
			$html .= join( '<br />', $shown_fields );
		}
		$html .= '<small><em>' . __( 'Wähle einen Beitragstyp aus und speichere die Einstellungen, um sie zu aktualisieren', 'rcpt' ) . '</em></small>';
		$html .= '</p>';
		$html .= '</div>';
		$html .= <<<EORcptJs
	...
	function widget( $args, $instance ) {
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				if ( $show_dates ) {
					echo '<span class="rcpt_item_date"><span class="rcpt_item_posted">' . __( 'Veröffentlich am', 'rcpt' ) . ' </span>' . get_the_date() . '</span>';
				}
				if ( $shown_fields ) {
